...
一、目标与完成度
- Use [metaphor_vehicle] as the overall structure to encode [data_tenor]; convey trend (not exact values)
- 产出概念草图（约30%完成度）：保留清晰的隐喻结构与数据映射关系，不追求像素级细节。
- 去数值、保关系：避免具体数值渲染，突出变量间的相对关系与趋势。
-axes-less (negative: no axes, no ticks, no gridlines)
...
-Axes-less layout：以隐喻结构承载信息；Negative：no axes, no ticks, no gridlines；如需对齐，仅允许极淡的对齐点/极短虚线（不得形成网格或坐标感）
